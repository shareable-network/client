window.publicationsRoute = () => {
    return {
        ...
        getEntryList: async function () {
            if (!this.$store.app.chainIsValid()) return;
            const core = Alpine.raw(await this.$store.app.getCore());
            const provider = Alpine.raw(await this.$store.app.getProvider());
            const url = 'https://api-testnet.snowtrace.io/api?module=logs&action=getLogs&address=0xf5Ab549c213b8FbE9e715f748868A3A3A5A6B3C3&topic0=0xca5832298b75b3645c83fda3dd6a040cba1dcb84367849a900217078ac7b8500&apikey=';
